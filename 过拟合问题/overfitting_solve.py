# -*- coding:utf-8 -*-
# @Author  : cs
# @Time    : 2018/8/13 10:30
# @File    : overfitting_solve
# @describe:
# 解决过拟合问题：L1正则、L2正则、弹性网络算法
# 为了解决过拟合问题，我们可以在损失函数中加入正则项（惩罚项），对系数过大的惩罚
# 对系数过多的也有一定惩罚能力，主要有：L1-norm（Lasso） 和 L2-norm（Ridge）
# Lasso回归：容易产生稀疏解，去掉冗余，速度快
# Ridge回归：稳定性好、准确性高
...
## 创建模拟数据
np.random.seed(100)
#显示方式设置suppress:是否使用科学计数法
np.set_printoptions(linewidth=1000,suppress=True)
N = 10
#np.random.randn:从标准正态分布中返回N个值
#np.random.rand:从[0,1)中返回一个或多个值
...
for i,d in enumerate(degree):
    plt.subplot(int(np.ceil(dm/2.0)),2,i+1)#把画布分成2*2的格子，把Y放在i+1格
...
plt.figure(facecolor='w')
degree = np.arange(1,N,2) #多项式扩展允许给定的阶数
dm = degree.size
colors = []
for c in np.linspace(16711680,255,dm):
    colors.append('#%06x'%int(c))
...
for t in range(4):
    ...
    for i,d in enumerate(degree):
        ...
        #判断lin对象中是否有对应的属性
        if hasattr(lin,'alpha_'): #判断lin这个模型组是否有alpha_这个属性
            idx = output.find(u'系数')
            output = output[:idx] + (u'alpha=%.6f, ' % lin.alpha_) + output[idx:]
